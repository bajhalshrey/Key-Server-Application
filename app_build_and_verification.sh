...
# 8. Automatic provisioning of custom Grafana dashboards.
...
PROMETHEUS_NAMESPACE="prometheus-operator" # Namespace for Prometheus and Grafana
...
HTTP_OVERVIEW_DASHBOARD_FILE="${GRAFANA_DASHBOARDS_DIR}/http-overview.json"
# The label the Grafana sidecar watches for dashboards
GRAFANA_DASHBOARD_WATCH_LABEL="grafana_dashboard=1"
# Temporary file for Grafana provisioning values
GRAFANA_PROVISIONING_VALUES_FILE="/tmp/grafana-dashboard-provisioning-values-$RANDOM.yaml"
...
cleanup_k8s_env() {
  ...
  log_info "Deleting any remaining grafana-dashboard-http-overview..."
  kubectl delete configmap grafana-dashboard-http-overview -n "${PROMETHEUS_NAMESPACE}" --ignore-not-found=true || true

  log_info "Deleting any remaining grafana-dashboard-key-generation..."
  kubectl delete configmap grafana-dashboard-key-generation -n "${PROMETHEUS_NAMESPACE}" --ignore-not-found=true || true

  log_info "Cleaning up temporary Helm values file..."
  rm -f "${GRAFANA_PROVISIONING_VALUES_FILE}" # Use the variable for cleanup
...
kubectl create configmap grafana-dashboard-http-overview \
  --from-file="${GRAFANA_DASHBOARDS_DIR}/http-overview.json" \
...
kubectl create configmap grafana-dashboard-key-generation \
  --from-file="${GRAFANA_DASHBOARDS_DIR}/key-generation.json" \
...
# Apply labels to the ConfigMaps for Grafana provisioning
log_info "Applying labels to Grafana Dashboard ConfigMaps..."
# Changed label from app.kubernetes.io/part-of to grafana_dashboard=1
kubectl label configmap grafana-dashboard-http-overview \
  "grafana_dashboard=1" \
  --namespace "${PROMETHEUS_NAMESPACE}" --overwrite || { log_error "Failed to label HTTP overview dashboard ConfigMap."; exit 1; }
...
(
  ...
  # CORRECTED: Reverted volume mount and env vars for Docker test to original working config
